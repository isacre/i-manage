...
export default function LeftSide({ menus, company, editMode = false }: Props) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      uploadImage(file, "company_images")
        .then((res) => {
          update({ ...company, image: res.url } as CompanyType)
        })
        .catch((err) => {
          console.log(err)
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative flex items-center gap-2">
        ...
        >
          ...
          <input accept="image/*" ref={imageInputRef} type="file" className="hidden" onChange={handleImageChange} />
        </div>
